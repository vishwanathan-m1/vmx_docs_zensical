...
<details close markdown="block"><summary id="page-on-tintcolor-property">pageOnTintColor Property
</summary>

* * *

This property applies a tint color to indicate the page that is currently in view.
...
This property is available only when [viewType](https://help.hcl-software.com/voltmx/v10/Iris/iris_widget_prog_guide/Content/Segment_Properties.html?h=segment#viewType) is set to pageview. The default pageOnTintColor is an opaque white dot.

*   This property is supported only on iOS 14 and later.

*   On iOS 14 and later, the page control displays an opaque version of the image provided through pageOnDotImage, without applying colors. iOS provides native support for coloring page dots, which you can enable in Volt Iris by setting the pageOnTintColor property in the preShow event.

When the Segment widget renders, the size of the dots is determined by the image specified in the pageOnDotImage property. The width and height of the dot match the resolution of the provided image. If you do not provide a custom image, Volt Iris displays the default page dot. To display a dot with a custom size, set the image for the dot in the widget properties.
...
<details close markdown="block"><summary id="page-off-tintcolor-property">pageOffTintColor Property</summary>

* * *

This property applies a tint color to indicate pages that are not currently in view.
...
This property is available only when [viewType](https://help.hcl-software.com/voltmx/v10/Iris/iris_widget_prog_guide/Content/Segment_Properties.html?h=segment#viewType) is set to pageview. The default pageOffTintColor is a translucent white (or gray) dot.


*   This property is supported only on iOS 14 and later.

*   On iOS 14 and later, the page control displays an opaque version of the image provided through pageOffDotImage, without applying colors. iOS provides native support for coloring page dots, which you can enable in Volt Iris by setting the pageOffTintColor property in the preShow event.

When the Segment widget renders, the size of the dots is determined by the image specified in the pageOffDotImage property. The width and height of the dot match the resolution of the image you provide. If you do not provide an image, Volt Iris displays the default page dot. To display a dot with a custom size, set the image for the dot in the widget properties.
