...
Starting with Iris 10.0.2, apps that target API level 36 or higher must support full‑screen design on Android. In Volt MX IRIS, this means the form’s background now extends beneath the status bar and navigation bar to give an edge‑to‑edge look on devices running Android 11 (API 30) and above.
