...
For more hands-on approach on how to implement Integration Services, import and preview the **News and Weather** app using Volt Iris.  
![](Resources/Images/Download_Button_08_202x33.png)

## Use Cases

If you want to develop an app which enables users to browse through news headlines with different categories such as _Top Stories_, _World_, _Science_, _Technology_, _Sports_, weather conditions and so on. In this scenario, you need to use Integration services of Volt Foundry to fetch data from 3rd party news and weather APIs, which populates the same data in the front end of the app.
...
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| a. **Request** A client sends a **request** to Volt Foundry.<br> b. **Filter**<br> c. **Message Parser (JSON, Form Encoded)**<br> d. **Pre-Processor** After Volt Foundry receives the call from the device, Volt Foundry executes custom Java/JS code based on the configured **Pre-Processor** before making any outbound calls. The custom Java/JS code is typically used to decide on what service to call. The custom code can also perform data validation on the request input.<br> e. **Request Builder (Velocity Template)** For adapters (such as XML, JSON), user can format the request sent to an enterprise backend by defining a [velocity template](http://velocity.apache.org/).<br> f. **Adapters** An adapter is the component that communicates to the backend. it takes the formatted request from the request builder and sends it to the enterprise back-end. In addition to the out of the box adapters, we can import adapters from [VoltMX Marketplace](https://marketplace.voltmx.com/domain/general?category=2124).<br> **_Note_:** You can select the **Enable pass-through** check box in the **Request Input** tab for Volt Foundry to forward the headers and body of clients request to the back end as is. |

| **Service Response flow**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| a. **Adapters** An adapter receives the response from the enterprise backend for further processing in Volt Foundry.<br> b. **Response Builder** Volt Foundry extracts data from the backend payload using **XPath** or **JSON Path** in the form of response parameters.<br> c.**Post-Processor** After extracting the data, Foundry executes the custom Java/JS code based on the configured **Post-Processor**.The custom Java/JS code is typically used to process the data before returning the data to the client. The custom code can also determine whether Foundry needs to make additional service calls.<br> c. **Message Formatter (JSON)**<br> e. **Filter** <br> f. **Response** Volt Foundry sends the transformed **response** (in JSON) to the client.<br> **_Note:_** You can select the **Enable pass-through** check box in the **Response Output** tab for Volt Foundry to forward the response from the back-end as is to clients. |
...
Out of the box Volt Foundry Integration Services support connectivity to the following different endpoint adapters.
...
<table>
  ...
  <tr>
    ...
    <td>JavaScript integrates plain JavaScript services to applications in Volt Foundry.</td>
    <td>
      <ul>
        <li>
  ...
  <tr>
    ...
    <td>MuleSoft (Anypoint Platform™) is a platform that helps app developers to design custom APIs and deploy to a Mule Enterprise Service Bus runtime (ESB). With MuleSoft integration service in Volt Foundry, developers can interact with more than 50 types of adapters.</td>
    <td>
      <ul>
        <li>
  ...
  <tr>
    ...
    <td>With Volt Foundry database adapter, you can connect to your own database as an endpoint. After you configure the database adapter in Volt Foundry Console, you can perform create, read, update, and delete (CRUD and Binary CRUD) operations on data in the tables and invoke stored procedures, functions, and views.</td>
    <td>
      <ul>
        <li>
  ...
  <tr>
    ...
    <td>With Volt Foundry MongoDB database adapter, you can connect to your own MongoDB database as an endpoint. After you configure the MongoDB adapter, you can perform create, read, update, and delete (CRUD) operations on MongoDB collections and documents.</td>
    <td>
      <ul>
        <li>
  ...
  <tr>
    ...
    <td>Volt Foundry allows you to access and use the external SAP services using custom SAP Connector. You must load the required Business Application Programming Interface (BAPI) functions to define an SAP service. The BAPI files contain the SAP methods and functions.</td>
    <td>
      <ul>
        <li>
...
<tr>
  ...
  <tr>
    <td>
      <a href="HCL_Universal_Orchestrator_.html">HCL Universal Orchestrator</a>
    </td>
    <td>HCL Universal Orchestrator framework backed by a large ecosystem of tools that helps you design, build, document, and consume your RESTful APIs.</td>
    <td>
      <ul>
        <li>
          <a href="HCL_Universal_Orchestrator_.html#configuring-an-HCL-Universal-Ochestrator-end-point-adapter">HCL Universal Orchestrator Endpoint Adapter</a>
        </li>
        <li>
          <a href="HCL_Universal_Orchestrator_.html#creating-operations-for-HCL-Universal-Orchestrator">Create Operations for HCL Universal Orchestrator</a>
          <ul>
            <li>
              <a href="HCL_Universal_Orchestrator_.html#configuring-request-parameters">Configure Request Operation for HCL Universal Orchestrator</a>
            </li>
            <li>
              <a href="HCL_Universal_Orchestrator_.html#configuring-response-parameters">Configure Response Operation for HCL Universal Orchestrator</a>
            </li>
          </ul>
        </li>
      </ul>
  ...
  <tr>
    <td>
      <a href="HCL_AION_.html">HCL AION</a>
    </td>
    <td>HCL AION framework backed by a large ecosystem of tools that helps you design, build, document, and consume your RESTful APIs.</td>
    <td>
      <ul>
        <li>
          <a href="HCL_AION_.html#configuring-an-HCL-AION-end-point-adapter">HCL AION Endpoint Adapter</a>
        </li>
        <li>
          <a href="HCL_AION_.html#creating-operations-for-HCL-AION">Create Operations for HCL AION</a>
          <ul>
            <li>
              <a href="HCL_AION_.html#configuring-request-parameters">Configure Request Operation for HCL AION</a>
            </li>
            <li>
              <a href="HCL_AION_.html#configuring-response-parameters">Configure Response Operation for HCL AION</a>
            </li>
          </ul>
        </li>
      </ul>
  ...
  <tr>
    ...
    <td>The Email Adapter is used to send emails from the client app. You can also view the status of the emails that were sent by using the Email service. The email requests are sent to the Volt Foundry server which in-turn redirects them to the Volt MX Engagement server.</td>
    <td>
      <ul>
        <li>
  ...
  <tr>
    ...
    <td>The AWS S3 service on Volt Foundry connects to the specified S3 bucket, and also performs supported operations.</td>
    <td>
      <ul>
        <li>
  ...
  <tr>
    ...
    <td>The Cloudian service on Volt Foundry connects to the specified S3 bucket on a HyperStore, and also performs supported operations.</td>
    <td>
      <ul>
        <li>
